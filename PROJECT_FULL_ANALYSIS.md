...
FlareGit is a full-stack platform designed to help developers enhance their GitHub presence. It allows users to authenticate via GitHub, customize portfolio profiles with custom colors, view their repository analytics, and generate professional repository and profile `README.md` files using Google's Gemini 2.0 Pro model via OpenRouter.

During our reverse-engineering audit, several critical architectural insights, design patterns, and high-level structural anomalies were identified:

1. **Dead Features and Blocked UI Components:**
   - The core functionality to select "Featured Projects" is supported by the Prisma Schema (`featuredProjects` JSON array) and fully implemented in the database update endpoints (`PUT /api/profile/[userId]`). The client-side component [repository-list.jsx](file:///j:/Saas/flaregit/src/components/repository-list.jsx) handles this interaction perfectly. However, this component is imported but **never rendered** in the dashboard page [page.js](file:///j:/Saas/flaregit/src/app/dashboard/page.js), rendering the featured projects feature inaccessible to the end-user.
   - Similarly, the components for generating and editing a user's _Profile_ README—[readme-generator.jsx](file:///j:/Saas/flaregit/src/components/readme-generator.jsx) and [readme-editor.jsx](file:///j:/Saas/flaregit/src/components/readme-editor.jsx)—exist and are imported in the main dashboard file, but are **never rendered** in the actual dashboard layout.
2. **Broken Navigation Links (404 Mismatch):**
   - The global navigation header [nav.jsx](file:///j:/Saas/flaregit/src/components/nav.jsx) attempts to link to the user's public portfolio page using `href={\`/u/\${session.user.username}\`}`. However, the actual Next.js routing directory is structured as `src/app/user/[username]`, meaning public profile routes are served under `/user/[username]`. Clicking "View Profile" from the navigation menu leads to a **404 Not Found** page.
3. **Incomplete Public Profile View:**
   - The public profile page at [page.js](file:///j:/Saas/flaregit/src/app/user/[username]/page.js) is heavily mocked. It ignores database values for generated READMEs, testimonials, or projects. Instead, it displays static mock statistics (12 Repositories, 1.2k Contributions, 8 Projects) and a static card reading _"Project management coming soon..."_.
...
| :---------------------- | :------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Runtime & Framework** | Next.js 14.1.0 (App Router), React 18.2                              | [package.json](file:///j:/Saas/flaregit/package.json)                                                                                               |
...
| **Markdown Rendering**  | `react-markdown`, `remark-gfm`, `rehype-raw`                         | Used inside [repository-readme-generator.jsx](file:///j:/Saas/flaregit/src/components/repository-readme-generator.jsx)                              |
...
| **AI Integration**      | OpenRouter Chat Completions (`google/gemini-2.0-pro-exp-02-05:free`) | Configured in [.env](file:///j:/Saas/flaregit/.env), processed in [ai-readme-generator.js](file:///j:/Saas/flaregit/src/lib/ai-readme-generator.js) |
| **HTTP Queries**        | Native `fetch` & Octokit SDK                                         | Used for GitHub API queries                                                                                                                         |
| **Validation**          | Zod 3.22.4                                                           | Declared in `package.json`                                                                                                                          |
...
- **AI & API Clients:** `octokit` (^3.2.1) (Official GitHub SDK), `stripe` (^14.10.0) (_Unused in implementation_).
...
- **Data Visualization & Analytics:** `recharts` (^2.10.4) (renders graphs), `react-calendar-heatmap` (^1.9.0) (renders contribution squares).
- **Markdown Support:** `react-markdown` (^9.0.3), `rehype-raw` (^7.0.0), `remark-breaks` (^4.0.0), `remark-gfm` (^1.0.0) (GitHub Flavored Markdown).
- **Utility Libraries:** `zod` (^3.22.4) (_Unused in current files, but present for schema validation_), `lucide-react` (^0.303.0) (Icons).
- **Drag and Drop:** `react-beautiful-dnd` (^13.1.1) (_Unused in current components_).
...
FlareGit follows a clean Next.js client-server boundary separation:
...
4. **Data Access Layer:** Local MongoDB databases are queried/updated using the Prisma ORM Client initialized in [prisma.js](file:///j:/Saas/flaregit/src/lib/prisma.js).
...
- `public/` - Holds static SVGs used by Next.js boilerplates.
...
- `next.config.mjs` - Empty Next.js customization file.
- `cleanup.js` - Utility script to delete retired repository routes.
...
│   ├── user/             # Public portfolio route: /[username]
...
│   ├── readme-generator.jsx# Profile README generation panel (Unused in UI)
│   ├── readme-editor.jsx # Profile README editor panel (Unused in UI)
│   ├── repository-list.jsx# Repository highlight toggles (Unused in UI)
...
├── hooks/                # Custom React hooks (contains duplicated use-toast.js)
...
- **Primary Workflow:**
  1. Accesses `http://localhost:3000/user/[username]` (public URL).
...
- **Primary Workflow:**
  ...
  4. Selects a repository via dialog, triggering AI generation of an enhanced project `README.md`.
  5. Reviews the draft, makes manual adjustments, and clicks "Save to GitHub" to push the update directly to the remote repository.
  6. Navigates to Theme Customization to pick primary, text, and card background colors for their public FlareGit page.
...
| **Repository README Generation**  | [repository-readme-generator.jsx](file:///j:/Saas/flaregit/src/components/repository-readme-generator.jsx) | `google/gemini-2.0-pro-exp-02-05:free` | **Complete**                                                                   |
...
| **Profile Theme Customizer**      | `PUT /api/profile/[userId]`                                                                                | N/A                                    | **Complete**                                                                   |
| **Profile URL Customization**     | `PUT /api/profile/[userId]`                                                                                | N/A                                    | **Complete**                                                                   |
| **Public Portfolio Page**         | [ProfilePage](file:///j:/Saas/flaregit/src/app/user/[username]/page.js)                                    | N/A                                    | **Partially Mocked** (hardcoded metrics, project grid says "coming soon")      |
| **Profile README Generation**     | [readme-generator.jsx](file:///j:/Saas/flaregit/src/components/readme-generator.jsx)                       | `google/gemini-2.0-pro-exp-02-05:free` | **Unused / Hidden** (endpoints exist, but component not rendered on dashboard) |
| **Profile README Editor**         | [readme-editor.jsx](file:///j:/Saas/flaregit/src/components/readme-editor.jsx)                             | N/A                                    | **Unused / Hidden** (component not rendered on dashboard)                      |
| **Featured Projects Selector**    | [repository-list.jsx](file:///j:/Saas/flaregit/src/components/repository-list.jsx)                         | N/A                                    | **Unused / Hidden** (component not rendered on dashboard)                      |
...
- **Auth Requirement:** Authenticated (redirects to `/auth/signin` on unauthenticated state)
- **Description:** Primary workspace. Organized via Tabs:
  - **Profile:** Contains read-only GitHub account items and forms to update user metadata (LinkedIn, Twitter, website, location, and custom URL).
  ...
  - **Settings:** Holds the primary/text/card custom color pickers and visual theme preview card.

### 10.5 Public Profile Page (`/user/[username]`)

- **File:** [page.js](file:///j:/Saas/flaregit/src/app/user/%5Busername%5D/page.js)
- **Auth Requirement:** None (Public)
- **Description:** Loads profile schema metrics matching `githubUsername` or `customUrl`. Colors are loaded dynamically from the custom theme stored in database.
- > [!WARNING]
  > The metrics (Repositories, Contributions, Projects) are currently hardcoded values. The project lists are represented by static placeholder texts.
...
- **Methods:** `GET`
- **Description:** Publicly fetches user profile parameters by querying either `githubUsername` or `customUrl`. Returns merged details of User and Profile tables.

### 11.3 AI Repository README Generator (`/api/ai/generate-repo-readme`)
...
### 11.4 AI Profile README Generator (`/api/github/generate-readme`)

- **File:** [route.js](file:///j:/Saas/flaregit/src/app/api/github/generate-readme/route.js)
- **Methods:** `GET`, `POST`
- **Description:**
  - `POST`: Analyzes authenticated developer profile, runs query aggregates via `GitHubProfileAnalyzer`, forwards metadata to `AIReadmeGenerator`, saves generated text in `generatedReadme` column, and returns raw text.
  - `GET`: Retrieves saved `generatedReadme` content and update timestamp from database.

### 11.5 Repository List (`/api/github/repositories/[username]`)
...
### 11.6 Repo README Fetcher (`/api/github/repositories/[username]/[repo]/readme`)
...
### 11.7 Push README to GitHub (`/api/github/repositories/[username]/[repo]/update-readme`)

- **File:** [route.js](file:///j:/Saas/flaregit/src/app/api/github/repositories/%5Busername%5D/%5Brepo%5D/update-readme/route.js)
- **Methods:** `POST`
- **Description:** Commits generated README markdown back to owner's GitHub repository.

### 11.8 User Profile Editor (`/api/profile/[userId]`)
...
- **Description:**
  ...
  - `PUT`: Updates user's `customUrl`, `location`, `website`, `twitter`, `linkedin`, and `customTheme` values.

### 11.9 GitHub Telemetry Proxies
...
- **Key Fields:**
  ...
  - `featuredProjects` (Json): Projects pinned by user (_Hidden in current UI_).
  - `generatedReadme`: Stored output of AI profile README text.
- **Relations:** Linked 1-to-1 with `User`. One-to-many with `Testimonial`.
...
- **Relations:** Linked to a parent `Profile`.
...
- **Scopes Configured:** `read:user user:email repo` in [auth.js](file:///j:/Saas/flaregit/src/lib/auth.js#L11).
  - _Security Note:_ The `repo` scope requests read/write access to both public and private repositories. This is necessary to query file structures and push files back to GitHub.
...
- **Auth Callbacks:**
  ...
  - `session()`: Exposes variables (`accessToken`, `user.username`, `user.id`, `user.bio`) directly to client components via `useSession()`.
...
- _Architecture Note:_ The developer dependencies include `@tanstack/react-query`, but it is **not used** in the application components. Data fetching relies on native `useEffect` fetch logic.
...
- **Global Navigation:** The [MainNav](file:///j:/Saas/flaregit/src/components/nav.jsx) header sits on top of all pages, featuring active page color changes, theme toggle commands, and dropdown option lists for logged-in sessions.
- **Custom UI Inputs:**
  - [color-picker.jsx](file:///j:/Saas/flaregit/src/components/ui/color-picker.jsx) combines quick-select color swatches with an HTML5 `<input type="color">` selector.
  - [ThemePreview](file:///j:/Saas/flaregit/src/components/theme-preview.jsx) renders a mockup preview card reflecting user-selected hex colors.
...
- Implemented in [AIReadmeGenerator](file:///j:/Saas/flaregit/src/lib/ai-readme-generator.js).
- It posts developer data packages to the OpenRouter endpoint `https://openrouter.ai/api/v1/chat/completions`, targeting the model `google/gemini-2.0-pro-exp-02-05:free`.
- System prompts configure strict formatting guidelines (HTML centers, shields.io grid badges, markdown headers, and blockquotes).
...
- `AI_API_ENDPOINT`: Set to `https://openrouter.ai/api/v1/chat/completions`.
- `AI_API_KEY`: OpenRouter API key.
- `RATE_LIMIT_MAX` & `RATE_LIMIT_WINDOW_MS`: Configured limits (inferred placeholders; no rate-limiter middleware is implemented in code).
- `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_PRO`, `STRIPE_PRICE_ID_ENTERPRISE`: Stripe subscription configuration settings (_Unused_).
...
### 18.1 Services (`src/lib/`)

- **[auth.js](file:///j:/Saas/flaregit/src/lib/auth.js):** Configures NextAuth. Customizes database sign-in callbacks to automatically synchronize GitHub account changes (bio, name, avatar) with MongoDB.
...
- **[ai-readme-generator.js](file:///j:/Saas/flaregit/src/lib/ai-readme-generator.js):** Customizes prompt generation templates. Regulates output layout and handles response cleaning (replaces multiple line breaks and spacing anomalies).

### 18.2 UI Components (`src/components/`)

- **[nav.jsx](file:///j:/Saas/flaregit/src/components/nav.jsx):** Implements header nav bar, system dark-mode toggling, and user sessions.
  - > [!IMPORTANT]
    > Contains a broken profile link: points to `/u/[username]` instead of `/user/[username]`.
...
- **[readme-generator.jsx](file:///j:/Saas/flaregit/src/components/readme-generator.jsx):** Implements profile README generation widgets.
  - > [!NOTE]
    > This component is unused and never rendered in the dashboard.
- **[readme-editor.jsx](file:///j:/Saas/flaregit/src/components/readme-editor.jsx):** Rich client-side editor containing a draft preview view.
  - > [!IMPORTANT]
    > This component is unused in the UI and uses a fragile regex-based markdown-to-HTML parser.
- **[repository-list.jsx](file:///j:/Saas/flaregit/src/components/repository-list.jsx):** UI grid interface to toggle "featured projects" tags.
  - > [!NOTE]
    > This component is unused and never rendered in the dashboard.
- **[repository-readme-generator.jsx](file:///j:/Saas/flaregit/src/components/repository-readme-generator.jsx):** The core active markdown generation component. It includes visual tab sections for editing and previewing code.

### 18.3 Root Pages (`src/app/`)

- **[page.js](file:///j:/Saas/flaregit/src/app/page.js):** Public landing page layout.
- **[page.js](file:///j:/Saas/flaregit/src/app/dashboard/page.js):** Core dashboard component coordinating custom theme pickers and API sync logic.
- **[page.js](file:///j:/Saas/flaregit/src/app/user/[username]/page.js):** Public developer portfolio viewer page.
  - > [!IMPORTANT]
    > Currently uses mocked values for developer statistics and project lists.
...
- **Robust Auth Flow Setup:** The custom sign-in callbacks in [auth.js](file:///j:/Saas/flaregit/src/lib/auth.js) handle database updates correctly, keeping MongoDB and GitHub avatars/credentials in sync.
- **Visual Polish:** Incorporates shadcn/ui styles, smooth CSS transitions, interactive hover settings, responsive layouts, and automatic dark/light theme switching.
...
- **Naïve Regex Markdown Parser:** [readme-editor.jsx](file:///j:/Saas/flaregit/src/components/readme-editor.jsx#L137-L158) attempts to convert markdown to HTML using simple regex replacements inside `dangerouslySetInnerHTML`. This approach is fragile and will break on complex elements, potentially leading to formatting errors or rendering issues. It contrasts with the robust markdown parsing used elsewhere in the app.
- **Dead Code and Unused Imports:** The dashboard page [page.js](file:///j:/Saas/flaregit/src/app/dashboard/page.js) imports several unused components:
  ...
  ```
  These unused imports increase bundle sizes and indicate incomplete feature branches.
- **Duplicated Code/Hooks:** The custom `use-toast` hook exists in duplicate locations:
  - `src/components/ui/use-toast.js` (standard shadcn path)
  - `src/hooks/use-toast.js`
    This duplication can lead to importing inconsistent versions across files.

---

## 21. Incomplete / Unclear / Inferred Areas

- **Unexposed Profile README Generation Features:** The backend routes and UI components (`ReadmeGenerator` and `ReadmeEditor`) for creating personal profile READMEs are fully implemented, but they are not exposed in the dashboard UI.
- **Unexposed Pinned/Featured Projects:** While the database, API handlers, and UI list components support selecting featured projects, this functionality is hidden from the user interface.
- **Mocked Public Profile Portfolio:** The public profile screen [page.js](file:///j:/Saas/flaregit/src/app/user/[username]/page.js) does not load dynamic project or testimonial lists from the database, displaying hardcoded metrics instead.
- **Missing Legal Pages:** The home page footer references `/privacy` and `/terms` links, but no corresponding routing folders or files exist in the project, leading to 404 errors.
- **No Rate Limiting:** The `.env` file references rate limit window variables, but no actual middleware or request interceptors implement rate limiting.
...
- Stack: Next.js 14.1.0 (App Router), Node (JS only), MongoDB via Prisma ORM, Tailwind CSS, Radix UI.
- Core Purpose: A developer portfolio builder and AI README generator. It uses GitHub OAuth to authorize access, analyzes profile data via Octokit (REST + GraphQL), and calls OpenRouter (Gemini 3 Flash) to generate markdown repository/profile READMEs.
- Key Services:
  ...
  - `src/lib/auth.js`: Manages NextAuth flows and syncs GitHub profile changes with MongoDB.
- Critical Bugs & Gaps:
  - Mismatch: `MainNav` links to `/u/[username]`, but public portfolios are served at `/user/[username]` (causes 404s).
  - Mocked UI: The public profile page `/user/[username]` uses hardcoded statistics and placeholders instead of database values.
  - Missing UI integrations: The dashboard imports but does not render `ReadmeGenerator`, `ReadmeEditor`, or `RepositoryList`. As a result, profile README generation and project pinning are inaccessible to users.
  - Incomplete Stripe implementation: References exist in the package config and schemas, but no integration code exists in `src`.
```

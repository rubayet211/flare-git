...
1. **Dead Features and Blocked UI Components:**
   ...
   * Similarly, the components for generating and editing a user's *Profile* README—[readme-generator.jsx](file:///j:/Saas/flaregit/src/components/readme-generator.jsx) and [readme-editor.jsx](file:///j:/Saas/flaregit/src/components/readme-editor.jsx)—exist and are imported in the main dashboard file, but are **never rendered** in the actual dashboard layout.
...
3. **Incomplete Public Profile View:**
   * The public profile page at [page.js](file:///j:/Saas/flaregit/src/app/user/[username]/page.js) is heavily mocked. It ignores database values for generated READMEs, testimonials, or projects. Instead, it displays static mock statistics (12 Repositories, 1.2k Contributions, 8 Projects) and a static card reading *"Project management coming soon..."*.
...
* **Utility Libraries:** `zod` (^3.22.4) (*Unused in current files, but present for schema validation*), `lucide-react` (^0.303.0) (Icons).
* **Drag and Drop:** `react-beautiful-dnd` (^13.1.1) (*Unused in current components*).
...
* **Key Fields:**
  ...
  * `featuredProjects` (Json): Projects pinned by user (*Hidden in current UI*).
...
- Core Purpose: A developer portfolio builder and AI README generator. It uses GitHub OAuth to authorize access, analyzes profile data via Octokit (REST + GraphQL), and calls OpenRouter (Gemini 2.0 Pro) to generate markdown repository/profile READMEs.
- Key Services:
